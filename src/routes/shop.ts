...
router.get(
    ...
    afterware(retrieve.getAroundShopList)
);

// 가게 구독 조회 API
router.get(
    "/:shopId/subscriptions",
    catchPrivilege(authorization, { shop: true }),
    afterware(retrieve.getShopSubsList)
);
...
router.get(
    "/:shopId/orders",
    catchPrivilege(authorization, { shop: true }),
    afterware(order.getShopOrderList)
);
...
// 가게 반찬 관련 API
router.post(
    "/:shopId/dishes/create",
    catchPrivilege(authorization, { shop: true }),
    afterware(dish.createShopDish)
);
router.get(
    "/:shopId/dishes",
    catchPrivilege(authorization, { all: true }),
    afterware(dish.getShopDishList)
);
router.get(
    "/:shopId/dishes/:dishId",
    catchPrivilege(authorization, { all: true }),
    afterware(dish.getShopDishDetail)
);
router.put(
    "/:shopId/dishes/:dishId",
    catchPrivilege(authorization, { shop: true }),
    afterware(dish.updateShopDish)
);
// *** 가게 대표사진 등록 / 취소 ***
...
router.delete(
    "/:shopId/dishes/:dishId",
    catchPrivilege(authorization, { shop: true }),
    afterware(dish.deleteShopDish)
);

// 가게 공지사항 관련 API *** 수정 필요 ***
router.post("/:shopId/posts/create", afterware(post.createShopPost));
router.get("/:shopId/posts", afterware(post.getShopPosts));
router.get("/:shopId/posts/:postId", afterware(post.getOnePost));
